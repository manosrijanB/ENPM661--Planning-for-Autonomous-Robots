...
def sample_unit_sphere(dim = 2):
    '''
    uniformly sample on a N-dimensional unit sphere
    Reference:
    ...
    Output:
        uniformly sampled points on a N-dimensional unit sphere
    '''
...
class HitAndRun:
  ...
  def __init__(self, dim, heuristic, start, goal, lower, upper):
    self.dim = dim
    self.start = start
    self.goal = goal
  ...
  def initialise(self):
    ...
    self.x_old = [self.start, self. goal][i]

  def sample(self, cbest):
    d = sample_unit_sphere(self.dim)

    lam_low = -cbest/2
    lam_high = cbest/2

    for _ in range(100):
      lam = np.random.uniform(lam_low, lam_high, 1) 

      x_new = self.x_old + (lam*d) 

      if self.heuristic(x_new, self.start, self.goal) < cbest:
        self.x_old = x_new
        return x_new
...
class Nonesampler:
  ...
  def sample(self, cbest):
      rnd = self.lower + np.random.rand(self.dim)*(self.upper - self.lower)
      return rnd
...
class Rejection:
  ...
  def sample(self, cbest):
    while True:
      rnd = self.lower + np.random.rand(self.dim)*(self.upper - self.lower)
      if self.heuristic(rnd, self.start, self.goal) < cbest:
        return rnd
...
def SampleNBall(center, radius, num = 1):
    '''
